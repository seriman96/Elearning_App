...
exports.getOverview = catchAsync(async (req, res, next)=>{
    ...
    const cours_BD = await Tour.find({category:'Big Data'});
    const cours_DS = await Tour.find({category:'Data Science'});
    const cours_DL = await Tour.find({category:'Deep Learning'}); 
    const cours_IOT = await Tour.find({category:'Internet of Things'});
    const cours_ML = await Tour.find({category:'Machine Learning'});
    const cours_NLP = await Tour.find({category:'Natural Language Processing'});
    const cours_PP = await Tour.find({category:'Python Programing'});
    const cours_CS = await Tour.find({category:'Cyber Security'});
    const cours_WD = await Tour.find({category:'Web Development'});
    // 2)  Build template
...
//manage user
  ...
  exports.getExam = async (req, res, next)=>{
    ...
    //const user_data = await User.find({type:'ADMIN'});
    //const course = await Tour.find();
    res.status(200).render('manageExams', {
      title: 'Manage Exams',
      //ud: user_data,
      //crs: course
    });
  ...
  exports.createExam = async (req, res, next)=>{
    ...
    //const user_data = await User.find({type:'ADMIN'});
    //const course = await Tour.find();
    res.status(200).render('createExams', {
      title: 'Create Exams',
      //ud: user_data,
      //crs: course
    });
  ...
  exports.top2cour = async (req, res, next)=>{
    
    //const user_data = await User.find({type:'ADMIN'});
    const course = await Tour.find();
    res.status(200).render('managecrs', {
      title: 'Find top-5 courses',
      //ud: user_data,
      tour: course
    });
  }; 

  exports.monthlycour = async (req, res, next)=>{
    
    //const user_data = await User.find({type:'ADMIN'});
    const course = await Tour.find();
    res.status(200).render('monthcrs', {
      title: 'Find Monthly courses',
      //ud: user_data,
      tour: course
    });
  };

  exports.courstats = async (req, res, next)=>{
    
    //const user_data = await User.find({type:'ADMIN'});
    const course = await Tour.find();
    res.status(200).render('statscrs', {
      title: 'Find course stats',
      //ud: user_data,
      tour: course
    });
  }; 
  ...
  exports.getCours = async (req, res, next)=>{
    ...
    const user_data = await User.find({type:'TRAINER'});
    //const course = await Tour.find();
  ...
  exports.searchCours = async (req, res, next)=>{
  ...
  exports.updateCour = async (req, res, next)=>{
  ...
  exports.uploadCour = async (req, res, next)=>{
    
    //const user_data = await User.find({$or : [{type:'TRAINER'}, {type:'ADMIN'}]});
    const course = await Tour.find();
    res.status(200).render('uploadCours', {
      title: 'uploading course image',
      //ud: user_data,
      tour: course
    });
  };
  ...
  exports.updateCours = catchAsync(async (req, res, next)=>{
  ...
  exports.getMyTours = async (req, res, next)=>{
    ...
    const tourIDs = bookings.map(el => el.cours); //return list of cours(tour) id
  ...
  exports.getMyReviews = async (req, res, next)=>{
    ...
    // 2) Find tours(course) with the returned IDs
    const tourIDs = reviews.map(el => el.cours); //return list of cours(tour) id
    const tours = await Tour.find({_id: { $in: tourIDs } }); //find tour(cours) which is included in the above tourIDs list
  ...
  exports.updateMyReviews = catchAsync(async (req, res, next)=>{
    ...
    const tours = await Tour.findById({_id: updatedReview.cours});
    //console.log(tours);
  ...
  exports.deleteMyReviews = catchAsync(async (req, res, next)=>{
    ...
    const tours = await Tour.findById({_id: deleteReview.cours});
    //console.log(tours);
