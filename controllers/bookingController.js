const stripe = require('stripe')('sk_test_51LHpTdSAQuXo2wkEcqaMHLSKAXrMpV0BlFxwT0QnNXzoI9ZFYlBqG118BEx1oDQx2tStbVEiN22MpAadMOisJm2E00E2UQAgSq'); //process.env.STRIPE_SECRET_KEY   //used to return a stripe object that we can work with it n it seems process.env.STRIPE_SECRET_KEY is not working here
...
exports.getCheckoutSession = catchAsync(async (req, res, next) => {
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        success_url: `${req.protocol}://${req.get('host')}/?cours=${req.params.tourID}&user=${req.user.id}&price=${tour.price}`,
        ...
        line_items: [
            {
                ...
                name: `${tour.name} Course`, //Tours when we're in tour project
...
exports.createBookingCheckout = catchAsync(async (req, res, next) => {
    ...
    const {cours, user, price} = req.query; //{cours, user}it's called structuring n req.query is the query string

    if(!cours && !user && !price) return next();
    await Booking.create({cours, user, price}); 
