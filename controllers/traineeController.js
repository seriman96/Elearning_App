var TraineeEnterModel = require('./../models/traineeEnterModel');
var TestPaperModel = require('./../models/testpaperModel');
...
var AnswersheetModel = require('./../models/subResultModel');
...
let traineeenter = (req,res,next)=>{
    ...
    if(errors){
        ...
        TestPaperModel.findOne({ _id : testid, isRegistrationavailable : true }).then((info)=>{
            if(info){
                TraineeEnterModel.findOne({$or:[{email : email , testid : testid},{contact : contact, testid : testid}]}).then((data)=>{
                    if(data){
                        res.json({
                            success : false,
                        ...
                        tempdata.save().then((u)=>{
                                sendmail(email,"Registered Successfully",`You have been successfully registered for the test. Click on the link given to take test  "${req.protocol + '://' + req.get('host')}/trainee/taketest?testid=${testid}&traineeid=${u._id}"`).then((dd)=>{
                                    console.log(dd)
                                }).catch((errr)=>{
                                    console.log(errr);
                                })
...
let resendmail = (req,res,next)=>{
    ...
    TraineeEnterModel.findById(userid,{email:1,testid : 1}).then((info)=>{
        if(info){
            console.log(info)
            sendmail(info.email,"Registered Successfully",`You have been successfully registered for the test. Click on the link given to take test  "${req.protocol + '://' + req.get('host')}/trainee/taketest?testid=${info.testid}&traineeid=${info._id}"`).then((dd)=>{
                console.log(dd)
            }).catch((errr)=>{ 
                console.log(errr);
            })
...
let TraineeDetails = (req,res,next)=>{
    ...
    TraineeEnterModel.findById(traineeid,{name:1,email:1,contact:1}).then((info)=>{
        if(info){
            res.json({
                success : true,
