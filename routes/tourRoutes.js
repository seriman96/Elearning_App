...
router.route('/montly-plan/:year').get(authController.protect, authController.restrictTo('ADMIN','TRAINER'), tourController.getMontlyPlan); //restricted for only normal people like 'admin','lead-guide', 'guide' here
...
.post(authController.protect, authController.restrictTo('ADMIN', 'TRAINER'), tourController.createTours); //tour creation limited by admin and lead-guide.
//.post(tourController.checkBody, tourController.createTours); //use of mutiple middleware

router
.route('/:id')
.get(tourController.getTours)
.patch(authController.protect, authController.restrictTo('ADMIN','TRAINER'), tourController.updateTours) //tour updation limited by admin and lead-guide
//.patch(authController.protect, authController.restrictTo('ADMIN','TRAINER'), tourController.uploadTourImages, tourController.resizeTourImages, tourController.updateTours) //tour updation limited by admin and lead-guide
.delete(authController.protect, authController.restrictTo('ADMIN','TRAINER'), tourController.deleteTours);//1st middlw for authentication(for user login),2nd middlw for authorization(here authorize admin n lead-guide for some action(delete action here)),last middlw for deleting purposes
