...
router.patch('/updateMe', userController.uploadUserPhoto, userController.resizeUserPhoto, userController.updateMe); //upload.single('photo') single here means we'll upload only 1 image n photo is the field which will receive the image n no need of authController.protect it gets replace by router.use(authController.protect);
