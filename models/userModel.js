...
const userSchema = new mongoose.Schema({
    ...
    type: {
        type: String,
        enum: ['LEARNER', 'TRAINER', 'ADMIN'], //list of authorize group of people for doing this role
        default: 'LEARNER'  //if nothing select by user this will be default value
    }, 
    password:{
        type: String,
    ...
    status: { //any user that is created new is of course an active user n used for user deleting purposes(route: /deleteMe) means don't completely delete user but disactivate.
      type: Boolean,
...
userSchema.pre('save', async function(next) {
  ...
  userSchema.pre('save', function(next) {
    //check if passwd has been modified or not n this.isNew will check if the document is new or not
  ...
  userSchema.pre(/^find/, function(next) { //^ means here every query starting by find that query middw will be applied on it 
    ...
    this.find({ status: { $ne: false } }); //return users where active is not equal to false means true 
    next();
  });
