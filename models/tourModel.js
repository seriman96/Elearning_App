...
const courSchema = new mongoose.Schema({
    name:{ //it's called schema type object
        ...
        required: [true, 'A course must have a name'], //make it require n if not filled or satisfied show error msg which is 2nd param n it's an inbuild validator
        unique: true, //set the field to unique
        trim: true,
        maxLength:[40, 'A course name must have less or equal than 40 characters'], //this attrib is also an inbuild validator n will set the maxi character for us
        minLength:[10, 'A course name must have more or equal than 10 characters'], //this attrib is also an inbuild validator n set minimum length character for us here 10
    ...
    price:{
        type: Number,
        required: [true, 'A course must have a number'] //the required attrib is called the validator bcs it's used to validate the data
    },
    priceDiscount:{
        type: Number,
        ...
        /*validate:{
            validator: function(val){
                ...
                return val < this.price; //val here is discount value n this.price is the price of current 
                //doc n when condition satisfied no validation else validation error will be fired
            },
    ...
    category:{
        type: String,
        //trim: true, //it's a schema type of string data type n used to remove the white space from begining n ending of the string
        required: [false, 'A course can have a category'], //A cours must have a category
    },
...
//as example
courSchema.index({price: 1, ratingsAverage: -1}); //compound index used to improve the query performance
//And when we create a compound index, no need to do for a single or individual indexing for every field. 
courSchema.index({slug: 1}); //applied indexing on slug field for searching performance on this field here n if we want an indexes we should do it on the database

//For being able to do queries on geospatial data we've to add index to the concerned field
//courSchema.index({startLocation: '2dsphere'}); //for geospatial data, indexing need to be the 2D sphere index if the data describes a real points on the earth like sphere n if we're using a fuctional points on a simple 2 dimensional plane. 
...
courSchema.virtual('durationWeeks').get(function(){
...
courSchema.virtual('reviews', {
    ref: 'Review', //table Review
    foreignField: 'cours', //the reference field for Tour docs in Review docs is tour field or foreign field of tour docs in Review docs n the correspondent field from tour docs in below
    localField: '_id' //is tour_id field here bcs tour reference field is tour in Review docs n it's _id field on it own
});

/*courSchema.virtual("averageRating").get(function() {
    return this.ratingsAverage > 0 ? this.ratingsAverage*(1.0) : 0.0;
});*/
...
courSchema.pre('save', function(next){ //this is pre-middleware n it can run before an actual event n the event here is save operation
...
courSchema.pre(/^find/, function(next){ //1st param means our middleware run for find() command either findOne() ... n find() method here will point to the current query n not to the current docs which is doc middleware
    this.find({secretCourse:{$ne: true}}); //find in current query where secretTour not equal to true
...
courSchema.post(/^find/, function(docs, next){ //1st param means our middlw run for find() command either findOne()... include all kind of find query n find() method here will point to the current query n not to the current docs which is doc middleware
...
const Tour = mongoose.model('Course', courSchema); //model is like a class in js n model name is Tour
